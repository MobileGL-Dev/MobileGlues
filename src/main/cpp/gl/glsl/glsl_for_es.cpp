...
static TBuiltInResource InitResources()
{
    static TBuiltInResource Resources = {
        32,6,32,32,64,4096,64,32,80,32,4096,32,128,8,16,16,15,
        -8,7,8,65535,65535,65535,1024,1024,64,1024,16,8,8,1,60,64,64,128,128,8,8,8,0,0,0,0,0,8,8,16,256,1024,1024,64,128,128,16,1024,4096,128,128,16,1024,120,32,64,16,0,0,0,0,8,8,1,0,0,0,0,1,1,16384,4,64,8,8,4,4,4,1,1,1,4,
...
int getGLSLVersion(const char* glsl_code) {
    ...
    std::smatch match;
    if (std::regex_search(glsl_code, match, version_pattern)) {
...
std::string forceSupporterOutput(const std::string& glslCode) {
    ...
    if (hasPrecisionFloat && hasPrecisionInt) {
        ...
        while (std::getline(iss, line)) {
            if (!(line.find("precision ") != std::string::npos && (line.find("float;") != std::string::npos || line.find("int;") != std::string::npos))) {
                oss << line << '\n';
            }
        }
        result = oss.str();
    ...
    size_t insertionPos = (lastExtensionPos != std::string::npos) ? result.find('\n', lastExtensionPos) + 1 : result.find('\n') + 1;
...
std::string process_uniform_declarations(const std::string& glslCode) {
    ...
    while (scan_pos < length) {
        if (glslCode.compare(scan_pos, 7, "uniform") == 0) {
            if (scan_pos > chunk_start) result.append(glslCode, chunk_start, scan_pos - chunk_start);
            size_t decl_start = scan_pos; scan_pos += 7;
            ...
            while (scan_pos < length) {
                ...
                for (const auto& kw : precision_kws) {
                    if (glslCode.compare(scan_pos, kw.length(), kw) == 0) {
                        precision = " " + kw; scan_pos += kw.length(); found_precision = true; break;
                    }
                }
                ...
                type = glslCode.substr(type_start, scan_pos - type_start); break;
            }
            while (scan_pos < length) {
                while (scan_pos < length && std::isspace(glslCode[scan_pos])) ++scan_pos;
                bool found = false;
                for (const auto& kw : precision_kws) {
                    if (glslCode.compare(scan_pos, kw.length(), kw) == 0) {
                        ...
                        scan_pos += kw.length(); found = true; break;
                    }
                }
                if (!found) break;
            }
            ...
            while (scan_pos < length && (std::isalnum(glslCode[scan_pos]) || glslCode[scan_pos] == '_')) ++scan_pos;
            std::string name = glslCode.substr(name_start, scan_pos - name_start);
...
std::string processOutColorLocations(const std::string& glslCode) {
    ...
    return std::regex_replace(glslCode, pattern, "\nlayout(location=$2) $1$2;");
}
...
bool process_non_opaque_atomic_to_ssbo(std::string& source) {
    ...
    static const std::regex decl_rx(R"(layout\s*\(\s*binding\s*=\s*(\d+)\s*(?:,\s*offset\s*=\s*(\d+)\s*)?\)\s*uniform\s+atomic_uint\s+(\w+)\s*;)", std::regex::icase);
    std::smatch m;
    ...
    while (std::regex_search(it, source.cend(), m, decl_rx)) {
        ...
        atomic_vars.insert(var); binding_map[var] = binding;
        std::string repl = "layout(std430, binding=" + binding + ") buffer AtomicCounterSSBO_" + binding + " {\n    uint " + var + ";\n};\n";
        source.replace(match_pos, m.length(0), repl);
        it = source.cbegin() + match_pos + repl.size();
    }
    if (atomic_vars.empty()) return true;
    for (auto& var : atomic_vars) {
        source = std::regex_replace(source, std::regex(R"(\batomicCounterIncrement\s*\(\s*)" + var + R"(\s*\))", std::regex::icase), "atomicAdd(" + var + ", 1u)");
        source = std::regex_replace(source, std::regex(R"(\batomicCounterDecrement\s*\(\s*)" + var + R"(\s*\))", std::regex::icase), "atomicAdd(" + var + ", uint(-1))");
        source = std::regex_replace(source, std::regex(R"(\batomicCounterAdd\s*\(\s*)" + var + R"(\s*,\s*([^)]+)\s*\))", std::regex::icase), "atomicAdd(" + var + ", $1)");
        source = std::regex_replace(source, std::regex(R"(\batomicCounter\s*\(\s*)" + var + R"(\s*\))", std::regex::icase), var);
    }
    static const std::regex rx_barrier(R"(([ \t]*\batomicAdd\b[^;]*;))", std::regex::icase);
    std::set<size_t> processed_positions;
    std::string result; size_t last_pos = 0;
    for (auto it = std::sregex_iterator(source.begin(), source.end(), rx_barrier); it != std::sregex_iterator(); ++it) {
        size_t start_pos = it->position();
        if (processed_positions.count(start_pos)) continue;
        result += source.substr(last_pos, start_pos - last_pos) + it->str() + "\n    memoryBarrierBuffer();";
...
void process_sampler_buffer(std::string& source) {
    ...
    while ((pos = source.find("isamplerBuffer", pos)) != std::string::npos) {
        source.replace(pos, 14, "isampler2D"); pos += 11;
    }
    static const std::regex pattern(R"(texelFetch\s*\(\s*(\w+)\s*,\s*([^)]+?)\s*\))");
    source = std::regex_replace(source, pattern, "texelFetch($1, ivec2(($2) % u_BufferTexWidth, ($2) / u_BufferTexWidth), 0)");
...
ivec2 bufferCoords(int index) {
    ...
    source = std::regex_replace(source, std::regex("texelFetch\\((\\w+)\\s*,\\s*ivec2\\(([^)]+)\\)\\s*,\\s*0\\)"), "texelFetch($1, bufferCoords($2), 0)");
    size_t insertion_point = find_insertion_point(source);
    if (insertion_point != std::string::npos) source.insert(insertion_point, boundaryProtection);
...
uniform int u_BufferTexHeight;
    ...
    if (insertion_point != std::string::npos) {
        ...
        if (insertion_point != std::string::npos) source.insert(insertion_point + 1, uniformDecl);
    }
}
...
static void inject_textureQueryLod(std::string& glsl) {
    ...
    static const std::string textureQueryLodImpl = R"(
...
static inline void inject_temporal_filter(std::string& glsl) {
    ...
    if (std::regex_search(glsl, defRegex)) return;
    static const std::string GI_TemporalFilterImpl = R"(
vec4 GI_TemporalFilter() {
    ...
    size_t insertPos = 0;
    static const std::regex uniformRegex(R"(^\s*(?:layout\s*\([^)]*\)\s*)?uniform\s+\w+(?:\s*\[\s*\d+\s*\])?\s+\w+(?:\s*\[\s*\d+\s*\])?\s*;.*$)", std::regex::ECMAScript | std::regex::multiline);
    for (auto it = std::sregex_iterator(glsl.begin(), glsl.end(), uniformRegex); it != std::sregex_iterator(); ++it) {
...
void inject_mg_macro_definition(std::string& glslCode) {
    static const std::string macro_definitions = "\n#define MG_MOBILEGLUES\n#define MG_MOBILEGLUES_VERSION " xstr(MAJOR) xstr(MINOR) xstr(REVISION) xstr(PATCH) "\n";
    size_t versionPos = glslCode.rfind("#version");
    size_t insertionPos = (versionPos != std::string::npos) ? glslCode.find('\n', versionPos) + 1 : glslCode.find('\n') + 1;
    glslCode.insert(insertionPos == std::string::npos ? 0 : insertionPos, macro_definitions);
}
